...
export async function generateTicketRESERVED(): Promise<User> {
...
export async function generateTicketNotIncludeHotel(): Promise<User> {
  const user = await createUser();
  const enrollment = await createEnrollmentWithAddress(user);
...
export async function generateTicketRemote(): Promise<User> {
  const user = await createUser();
  const enrollment = await createEnrollmentWithAddress(user);
